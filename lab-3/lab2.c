...
void* justDoIt(void*threadData){
	struct thread_info* info=(struct thread_info*) threadData;
...
int main(int argc, char* argv[])
{
	...
	strcpy(_directory_root,argv[3]);
      ...
      for(int i=0;i<numThreads;i++){
      	...
      	pthread_create(&threads[i],NULL, justDoIt, (void*) &all_thread_info[i]);
      	sem_post(&work_mutex);
      }
